...
mock.onGet('/api/lists-app/lists').reply(config => {
	const id = config.params.id;
	let response = [];
	switch (id) {
		case 'frequent': {
			response = listsDB.lists.filter(list => listsDB.user[0].frequentLists.includes(list.id));
			break;
		}
		case 'starred': {
			response = listsDB.lists.filter(list => listsDB.user[0].starred.includes(list.id));
			break;
		}
		default: {
			response = listsDB.lists;
		}
	}
	return [200, response];
});

mock.onGet('/api/lists-app/user').reply(config => {
	return [200, listsDB.user[0]];
});

mock.onPost('/api/lists-app/add-list').reply(request => {
	...
	return [200, listsDB.lists];
});

mock.onPost('/api/lists-app/update-list').reply(request => {
	const data = JSON.parse(request.data);

	listsDB.lists = listsDB.lists.map(list => {
...
mock.onPost('/api/lists-app/remove-list').reply(request => {
	const data = JSON.parse(request.data);

	listsDB.lists = listsDB.lists.filter(list => data.listId !== list.id);

	return [200, listsDB.lists];
});

mock.onPost('/api/lists-app/remove-lists').reply(request => {
	const data = JSON.parse(request.data);
	listsDB.lists = listsDB.lists.filter(list => !data.listIds.includes(list.id));
	return [200, listsDB.lists];
});

mock.onPost('/api/lists-app/toggle-starred-list').reply(request => {
	const data = JSON.parse(request.data);
	listsDB.user[0].starred = _.xor(listsDB.user[0].starred, [data.listId]);
	return [200, listsDB.user[0]];
});

mock.onPost('/api/lists-app/toggle-starred-lists').reply(request => {
	const data = JSON.parse(request.data);
	listsDB.user[0].starred = _.xor(listsDB.user[0].starred, data.listIds);
	return [200, listsDB.user[0]];
});

mock.onPost('/api/lists-app/set-lists-starred').reply(request => {
	const data = JSON.parse(request.data);

	listsDB.user[0].starred = [...listsDB.user[0].starred, ...data.listIds];

	return [200, listsDB.user[0]];
});

mock.onPost('/api/lists-app/set-lists-unstarred').reply(request => {
	const data = JSON.parse(request.data);

	listsDB.user[0].starred = listsDB.user[0].starred.filter(listId => !data.listIds.includes(listId));
