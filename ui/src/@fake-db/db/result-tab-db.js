...
mock.onGet("/api/search-app/results").reply(() => {
    return [200, resultTabDB.results];
});

mock.onGet("/api/search-app/result").reply(request => {
    const { resultId } = request.params;
    const response = _.find(resultTabDB.results, { id: resultId });
    return [200, response];
});

mock.onPost("/api/search-app/result/save").reply(request => {
    const data = JSON.parse(request.data);
    let result = null;

    resultTabDB.results = resultTabDB.results.map(_result => {
        if (_result.id === data.id) {
            result = data;
            return result;
        }
        return _result;
    ...
    if (!result) {
        result = data;
        resultTabDB.results = [...resultTabDB.results, result];
    }

    return [200, result];
});

mock.onGet("/api/search-app/orders").reply(() => {
    return [200, resultTabDB.orders];
});

mock.onGet("/api/search-app/order").reply(request => {
    const { orderId } = request.params;
    const response = _.find(resultTabDB.orders, { id: orderId });
    return [200, response];
});
