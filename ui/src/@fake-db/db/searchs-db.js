			...
			groups: [
				{
					...
					name: 'Friends',
					listIds: ['5725a680bbcec3cc32a8488a', '5725a680e87cb319bd9bd673', '5725a6802d10e277a0f35775']
				},
				{
...
mock.onGet('/api/searchs-app/searchs').reply(config => {
	const id = config.params.id;
	let response = [];
	switch (id) {
		case 'frequent': {
			response = searchsDB.searchs.filter(list => searchsDB.user[0].frequentLists.includes(list.id));
			break;
		}
		case 'starred': {
			response = searchsDB.searchs.filter(list => searchsDB.user[0].starred.includes(list.id));
			break;
		}
		default: {
			response = searchsDB.searchs;
		}
	}
	return [200, response];
});

mock.onGet('/api/searchs-app/user').reply(config => {
	return [200, searchsDB.user[0]];
});

mock.onPost('/api/searchs-app/add-list').reply(request => {
	...
	return [200, searchsDB.searchs];
});

mock.onPost('/api/searchs-app/update-list').reply(request => {
	const data = JSON.parse(request.data);

	searchsDB.searchs = searchsDB.searchs.map(list => {
...
mock.onPost('/api/searchs-app/remove-list').reply(request => {
	const data = JSON.parse(request.data);

	searchsDB.searchs = searchsDB.searchs.filter(list => data.listId !== list.id);

	return [200, searchsDB.searchs];
});

mock.onPost('/api/searchs-app/remove-searchs').reply(request => {
	const data = JSON.parse(request.data);
	searchsDB.searchs = searchsDB.searchs.filter(list => !data.listIds.includes(list.id));
	return [200, searchsDB.searchs];
});

mock.onPost('/api/searchs-app/toggle-starred-list').reply(request => {
	const data = JSON.parse(request.data);
	searchsDB.user[0].starred = _.xor(searchsDB.user[0].starred, [data.listId]);
	return [200, searchsDB.user[0]];
});

mock.onPost('/api/searchs-app/toggle-starred-searchs').reply(request => {
	const data = JSON.parse(request.data);
	searchsDB.user[0].starred = _.xor(searchsDB.user[0].starred, data.listIds);
	return [200, searchsDB.user[0]];
});

mock.onPost('/api/searchs-app/set-searchs-starred').reply(request => {
	const data = JSON.parse(request.data);

	searchsDB.user[0].starred = [...searchsDB.user[0].starred, ...data.listIds];

	return [200, searchsDB.user[0]];
});

mock.onPost('/api/searchs-app/set-searchs-unstarred').reply(request => {
	const data = JSON.parse(request.data);

	searchsDB.user[0].starred = searchsDB.user[0].starred.filter(listId => !data.listIds.includes(listId));
