...
function SearchPageBreadcrumb({ className }) {
	const { pathname } = useLocation();
	const pathArr = getPathTree(SearchNavigation, pathname);

	return (
		<div className={clsx('', className)}>
			<Breadcrumbs aria-label="breadcrumb">
				<Link className="font-bold hover:underline" color="secondary" to="/apps/search" role="button">
					Search
				</Link>
				{pathArr.map(item => (
