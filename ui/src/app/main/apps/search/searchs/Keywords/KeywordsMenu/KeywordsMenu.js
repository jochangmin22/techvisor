...
import { setKeywordsOptions } from 'app/main/apps/search/store/searchsSlice';
import React, { useState } from 'react';

function KeywordsMenu() {
	const dispatch = useDispatch();
	const keywordsOptions = useSelector(({ searchApp }) => searchApp.searchs.analysisOptions.keywordsOptions);
	const [anchorEl, setAnchorEl] = useState(null);

	const { form, handleChange } = useForm(keywordsOptions);
	...
	function handleSubmit(ev) {
		...
		dispatch(setKeywordsOptions(form));
		handleMenuClose();
	}

	return (
		<>
			<Tooltip title="핵심주제어 설정">
