...
import Pfs from './components/Pfs';
	...
	return (
		<div className={clsx(classes.root, 'flex flex-col w-full h-full')}>
			<div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-8">
				...
				>
					<Tab label="종합" className={classes.tab} {...a11yProps(0)} />
					<Tab label="CPP" className={classes.tab} {...a11yProps(1)} />
					<Tab label="PII" className={classes.tab} {...a11yProps(2)} />
			...
			<TabPanel value={tabValue} index={1}>
				{tabValue === 1 &&
					(isEmpty ? (
						<EmptyMsg icon="photo" msg="피인용도 지수 CPP" />
					) : (
						<div className="flex w-full">
							<Cpp searchText={searchText} />
						</div>
					))}
			</TabPanel>
			<TabPanel value={tabValue} index={2}>
				{tabValue === 2 &&
					(isEmpty ? (
						<EmptyMsg icon="layers" msg="PII" />
					) : (
						<div className="flex w-full">
							<Pii searchText={searchText} />
						</div>
					))}
			</TabPanel>
